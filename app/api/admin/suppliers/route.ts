...
export async function PATCH(request: NextRequest) {
  ...
  const { id, catalog_mode, checkout_enabled, ...settings } = parsed.data;
  ...
  const { data, error } = await db.from('suppliers').update({ ...settings, updated_at: new Date().toISOString() }).eq('id', id).select('*').single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
